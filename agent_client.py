...
Assumptions for the real integration (adjust later):
...
def _build_payload(request: BriefingRequest) -> dict[str, Any]:
    return {
        ...
        # When Tavily/search is available, attach the web_search tool here:
...
async def _call_openclawd(request: BriefingRequest) -> str:
    """Real OpenClawD HTTP call — enable when credentials are configured."""
...
async def call_beacon_lab_agent(request: BriefingRequest) -> str:
    ...
    # TODO: Replace stub with real OpenClawD call once credentials are available.
    # return await _call_openclawd(request)

    return (
        f"[STUB] Beacon Lab Daily Threat Briefing — {request.date.isoformat()}\n\n"
        f"Language: {request.language} | Max words: {request.max_words}\n\n"
        "Executive summary: This is a placeholder briefing generated by the API stub.\n\n"
        "• Top threats: Replace agent_client stub with _call_openclawd() for live intel.\n"
        "• Vulnerabilities: No live data in stub mode.\n"
        "• Recommendations: Configure OPENCLAWD_BASE_URL and OPENCLAWD_API_KEY."
    )
